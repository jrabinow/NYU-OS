...
/*
 * No need to read ALL random values into array at once: if we only need 3
 * random values, why read the whole file?
 * 32 values are read into the array at start. Every time we need an additional
 * $ 2^n+1 $th random number, we try to read array_size * 2 more random numbers
 * from the file.
 * To avoid static variables as much as possible, array_size is saved in
 * array[-1], and the offset into the file is saved in array[-2].
 * Once the file is empty, emptied_file is set to true and array[-2] represents
...
#define needs_more_mem(array)		((array)[-1] >= START_MEM &&\
		((array)[-1] & ((array)[-1] - 1)) == 0)
...
void init_random(char *input_path)
{
	...
	randvals = (unsigned long*) xmalloc((START_MEM + 2) *
			sizeof(unsigned long)) + 2;
	randvals[-1] = 0;

	/* ignore number of random values in file. We don't need it */
	if(fscanf(input, "%*u") != 0) {
		...
		if(fscanf(input, "%lu", &randvals[i]) == EOF) {
			switch(ferror(input)) {
				case 0:	/* real end of file */
					randvals[-2] = i;
					emptied_file = true;
					randvals = (unsigned long*)
						xrealloc(randvals - 2, (2 + i)
						* sizeof(unsigned long)) + 2;
					break;
...
void fill_randvals_array(void)
{
	...
	randvals = (unsigned long*) xrealloc(randvals - 2, (upper_bound + 2) *
			sizeof(unsigned long)) + 2;
