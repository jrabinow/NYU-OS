...
function main()
{
	...
	local input_testdir=./inputs			# directory containing sample inputs
							# THIS DIRECTORY SHOULD NOT CONTAIN ANYTHING ELSE
	local output_testdir=./references		# directory containing reference outputs
	...
	local show_all=true				# ignore (deactivate) suppress* arrays and print all errors
	local tmpfile=$(mktemp)

	# non-generic variables
	local rfile=../inputs/rfile			# path to file containing random numbers
	...
	while getopts "b:d:e:himotv" opt; do
		case ${opt} in
			b)	# redefine binary
				binary="${OPTARG}"
				;;
