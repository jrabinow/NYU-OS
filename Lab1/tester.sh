...
function main()
{
	...
	local input_testdir=labsamples			# directory containing sample inputs
	local output_testdir=labsamples			# directory containing reference outputs
...
EOF
	...
	if $benchmark; then
		...
		for((i = 0; i < $count; i++)) {
			...
			milli=$(echo ${output##*m*.0}|cut -ds -f1)
